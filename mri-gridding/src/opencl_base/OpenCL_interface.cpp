...
void OpenCL_interface (
  ...
  memset(maxIntData+n, 0xEE, (((n+3)&~(3))-n)*sizeof(unsigned int));
  ...
  cl_int clStatus;
  OCL_ERRCK_RETVAL ( clStatus = clBuildProgram(gpu_kernels, 1, &clDevice, compileOptions, NULL, NULL) );
  ...
  pb_sig_clmem("sortedSample", clCommandQueue, sortedSample_d, T_FLOAT);
...
*/
  ...
  pb_sig_clmem("sortedSample", clCommandQueue, sortedSample_d, T_FLOAT);
  pb_sig_clmem("binStartAddr", clCommandQueue, binStartAddr_d, T_INT);
  pb_sig_clmem("gridData_d", clCommandQueue, gridData_d, T_INT);
  pb_sig_clmem("sampleDensity", clCommandQueue, sampleDensity_d, T_FLOAT);
